...
def worker(ks, lambda_list=None, stype=None, Uindex=None, SED_dict=None, redshift=None):
    ks = [k for k in ks if k is not None]
    nuLnu     = np.zeros([len(ks), len(lambda_list)])
    lambda_rest = lambda_list / (1 + np.array(redshift)[ks, np.newaxis]) * u.um #lambda list is in micron!
    nu_rest_Hz = (cst.c * u.m/u.s) / lambda_rest.to(u.m)
    for i, k in enumerate(ks):
        nuLnu[i] = np.interp(lambda_rest[i].value, SED_dict["lambda"], SED_dict[stype[k]][Uindex[k]])
...
def gen_Snu_arr(lambda_list, SED_dict, redshift, LIR, Umean, Dlum, issb):
    ...
    Worker = partial(worker, lambda_list=lambda_list, stype=stype, Uindex=Uindex, SED_dict=SED_dict, redshift=redshift)
    pool = Pool(cpu_count())
